...
#######################
#######################
##Port_forward_to_NGINX
iptables -t nat -A PREROUTING -p tcp -i eth0 --dport 80 -j DNAT --to-destination 10.0.0.10:80
##Port_forward_to_Portainer
iptables -t nat -A PREROUTING -p tcp -i eth0 --dport 49020 -j DNAT --to-destination 10.0.0.5:9000
##Port_Forward_to_PhpMyAdmin
# iptables -t nat -A PREROUTING -p tcp -i eth0 --dport 48080 -j DNAT --to-destination 172.20.0.2:80


#Allow all connection between csf.allow and NGINX container
iptables -A DOCKER -m set --match-set chain_ALLOW src -j ACCEPT
#Allow connection between csf.conf (Region AZ (cc_az) and ports in CC_ALLOWPORTS
iptables -A DOCKER -m set --match-set cc_az src -j CC_ALLOWPORTS
